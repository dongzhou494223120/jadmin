...
function setTrHeight(){
	...
	for(var i=0;i<rows.length;i++){
		rows[i].style.height=(tbodyheight/rows.length-4)+"px";
		var tds=rows[i].getElementsByTagName("td");
		for(var j=0;j<tds.length;j++){
			var a=tds[j].childNodes[0];
			a.style.width=(tds[j].offsetWidth-10)+"px";
			a.style.height=(tds[j].offsetHeight-9)+"px";
			a.style.lineHeight=(tds[j].offsetHeight-9)+"px";
		}
	}
}
//得到id对象
...
//新增便签
function aboluoAdd(){
	var value = $('#aboluo-textarea').val();
	var date = $('#aboluo-textarea').attr('_date');
	if(value == ''){
		return;
	}
	$('#aboluo-textarea').val('');
	$('#aboluo-textarea').attr('_id', '');
	$('#aboluo-textarea').attr('_date', getNowFormatDate());
	parent.addMemo(date, value);
}

//保存便签内容
function savememo(){
	var value = $('#aboluo-textarea').val();
	var id = $('#aboluo-textarea').attr('_id');
	$.ajax({
		   type:"POST",
	   	   url: '/memoAjax/save',
		   data:{"id":id,"value":value}, //这里用ajax可以改变状态
		   async:false,
		   success:function(json){
			   if(json.status){
			   	$('#aboluo-textarea').attr('_id', json.id);
			   }else{
			   	console.log('保存异常');
			   }
		   },
...
$(document).ready(function(){
  $.ajax({
		   ...
		   success:function(json){
			   	$('#aboluo-textarea').attr('_id', json.id);
			   	$('#aboluo-textarea').attr('_date', json.date);
			   	$('#aboluo-textarea').val(json.value);
		   },
		   error:function(json){
			console.log('获取异常');
		   }
